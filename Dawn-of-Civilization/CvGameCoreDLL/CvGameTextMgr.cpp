...
//---------------------------------------------------------------------------------------
//mediv01 20200814 主要是界面文本显示的内容 里面更改了选中单位显示城市名称
...
{
	if (GET_TEAM(GET_PLAYER(ePlayer).getTeam()).isHasTech((TechTypes)CALENDAR) || iGameTurn < GET_PLAYER(ePlayer).getBirthTurn()) //mediv01 需要先研究历法才能看日期
...
/*************************************************************************************************/
	...
	if (GC.getDefineINT("CVGAMETEXT_SHOW_ENERY_ATTACK_COMBAT_INFO") == 1) { //此参数废弃，尚未完成
		szString.append(NEWLINE);
		szString.append(L'【防御敌人进攻的战斗预估】');
		pAttacker = gDLL->getInterfaceIFace()->getSelectionList()->AI_getBestGroupAttacker(pPlot, false, iOdds);//我方进攻部队
		...
		{
			// BUG - Combat Odds for Friendlies - start
			pDefender = pPlot->getBestDefender(NO_PLAYER, pAttacker->getOwnerINLINE(), pAttacker, !gDLL->altKey(), NO_TEAM == pAttacker->getDeclareWarMove(pPlot));//敌方防守部队
			// BUG - Combat Odds for Friendlies - end
	...
	//					}
						...
						if (GC.getDefineINT("CVGAMETEXT_SHOW_ENERY_ATTACK_COMBAT_INFO_PART2") == 1) {
							if (ACO_debug)
							{
								szTempBuffer.Format(L"E[HP ATTACKER]");
								//szString.append(NEWLINE);
						...
						if (GC.getDefineINT("CVGAMETEXT_SHOW_ENERY_ATTACK_COMBAT_INFO_PART3") == 1) {
						...
						if (GC.getDefineINT("CVGAMETEXT_SHOW_ENERY_ATTACK_COMBAT_INFO_PART4") == 1) {
							if (iView & getBugOptionINT("ACO__ShowSurvivalOdds", 3, "ACO_SHOW_SURVIVAL_ODDS"))
							{
								szTempBuffer.Format(L"%.2f%%", 100.0f * (AttackerKillOdds + RetreatOdds + PullOutOdds));
								szTempBuffer2.Format(L"%.2f%%", 100.0f * (RetreatOdds + PullOutOdds + DefenderKillOdds));
						...
						if (GC.getDefineINT("CVGAMETEXT_SHOW_ENERY_ATTACK_COMBAT_INFO_PART5") == 1) {
							szString.append(L"<img=Art/ACO/green_bar_left_end.dds>");
							for (int i = 0; i < fullBlocks; ++i)
							{
								szString.append(L"<img=Art/ACO/green_bar_10.dds>");
						...
						//START ATTACKER DETAIL HP HERE
						// Individual bars for each attacker HP outcome.
						if (GC.getDefineINT("CVGAMETEXT_SHOW_ENERY_ATTACK_COMBAT_INFO_PART6") == 1) {
							if (iView & getBugOptionINT("ACO__ShowAttackerHealthBars", 2, "ACO_SHOW_ATTACKER_HEALTH_BARS"))
							{
								for (int n_A = 0; n_A < iNeededRoundsDefender - 1; n_A++)
								{
						...
						szString.append(NEWLINE);
						if (GC.getDefineINT("CVGAMETEXT_SHOW_ENERY_ATTACK_COMBAT_INFO_PART7") == 1) {
						if (iView & getBugOptionINT("ACO__ShowBasicInfo", 3, "ACO_SHOW_BASIC_INFO"))
						{
							szTempBuffer.Format(SETCOLR L"%d" ENDCOLR L", " SETCOLR L"%d " ENDCOLR,
								TEXT_COLOR("COLOR_POSITIVE_TEXT"), iDamageToDefender, TEXT_COLOR("COLOR_NEGATIVE_TEXT"), iDamageToAttacker);
	...
	/*************************************************************************************************/
				...
				*/
				...
				if (GC.getDefineINT("CVGAMETEXT_SHOW_ENERY_ATTACK_COMBAT_INFO_PART8") == 1) {
				...
				if (pPlot->isCore((PlayerTypes)iI))
				{
					if (GET_PLAYER((PlayerTypes)iI).isAlive() || GET_PLAYER((PlayerTypes)iI).canEverRespawn()|| GC.getDefineINT("GAME_TEXT_SHOW_FOREGIN_CORE") ==1)
					{
						if (GC.getDefineINT("GAME_TEXT_SHOW_FOREGIN_CORE") == 1) {
							if (bForeignCore)
							{
								sForeignName.append(CvWString::format(L"  %s", GET_PLAYER((PlayerTypes)iI).getCivilizationDescription()));
							}
							else
							{
								sForeignName.append(CvWString::format(L":  %s", GET_PLAYER((PlayerTypes)iI).getCivilizationDescription()));
			...
			if (GC.getDefineINT("GAME_TEXT_SHOW_CITY_NAME_IN_ALL_UNIT") == 1) //mediv01 所有单位都显示城市名称
			{
				if (!pPlot->isWater() && !pPlot->isPeak())
				{
					...
					if (!szName.empty())
					{
						...
						if (GC.getDefineINT("CVGAMETEXT_SHOW_CONNECT_TO_APITAL") == 1 || 1 == 1) {
			...
			if (GC.getDefineINT("GAME_TEXT_SHOW_RELIGION_MAP") == 1) {//mediv01 显示宗教数值
				for (iI = 0; iI < NUM_RELIGIONS; iI++)
				{
					...
					if (iReligionFactor == REGION_SPREAD_NONE) {
						...
						StringReligionFound.append(CvWString::format(L"  少数区"));
						
					}
					else if (iReligionFactor == REGION_SPREAD_PERIPHERY) {
						StringReligionFound.append(CvWString::format(L"  边缘区"));
				...
				if (pIntList1.size() > 0) {
					...
					for (int i = 1; i <= pIntList1.size(); i++) {
						...
						if (PlayerNum == 0) {
							...
							szTempBuffer.Format(L"  加勒比贸易公司  ");
						}
						else if (PlayerNum == 2) {
							szTempBuffer.Format(L"  南撒哈拉贸易公司  ");
						}
						else if (PlayerNum == 3) {
							szTempBuffer.Format(L"  东南亚贸易公司  ");
						}
						else {
							szTempBuffer.Format(L"");
						}
					...
					for (int i = 1; i <= pIntList1.size(); i++) {
						...
						if (GC.getDefineINT("CVGAMETEXT_AIWAR_IN_MAP_SHOW_ANCIENT") == 1) {
							if (PlayerNum == 0) {
								...
								szTempBuffer.Format(L"  公元前400年亚历山大征服巴比伦  ");
							}
							else if (PlayerNum == 6) {
								szTempBuffer.Format(L"  公元前400年亚历山大征服埃及  ");
							}
							else if (PlayerNum == 7) {
								szTempBuffer.Format(L"  公元前400年亚历山大征服波斯  ");
							}
							else if (PlayerNum == 8) {
								szTempBuffer.Format(L"  公元880年泰米尔征服印尼  ");
							}
							else if (PlayerNum == 9) {
								szTempBuffer.Format(L"  公元1200年西班牙征服摩尔  ");
							}
							else if (PlayerNum == 10) {
								szTempBuffer.Format(L"  公元1000年突厥征服波斯  ");
							}
							else if (PlayerNum == 11) {
								szTempBuffer.Format(L"  公元1100年突厥征服小亚细亚  ");
							}
							else if (PlayerNum == 12) {
								szTempBuffer.Format(L"  公元1220年蒙古征服中亚  ");
							}
							...
							else if (PlayerNum == 15 || PlayerNum == 16 || PlayerNum == 17) {
								szTempBuffer.Format(L"  公元1095年英、法、神罗十字军东征 ");
							}
							else if (PlayerNum == 18 || PlayerNum == 19 || PlayerNum == 20) {
								szTempBuffer.Format(L"  公元1170年萨拉丁的崛起 ");
							}
							else if (PlayerNum == 51) {
								szTempBuffer.Format(L"  公元前220年汉尼拔远征罗马  ");
							}
							else if (PlayerNum == 52) {
						...
						int PlayerNum_Rec;
						if (PlayerNum == 15 || PlayerNum == 16 || PlayerNum == 17) {
							PlayerNum_Rec = 15;
						}
						else if (PlayerNum == 18 || PlayerNum == 19 || PlayerNum == 20) {
							PlayerNum_Rec = 18;
						}
						else if (PlayerNum >= 21 && PlayerNum <= 24) {
							PlayerNum_Rec = 21;
						}
						else if (PlayerNum >= 25 && PlayerNum <= 28) {
							PlayerNum_Rec = 25;
						...
						mapStudent.insert(pair<int, string>(1, "student_zero"));
						...
						*/
						...
						/*
						list<int>::iterator itr = record_list.begin();
						for (itr = record_list.begin(); itr != record_list.end(); ++itr) {
							if (itr == PlayerNum_Rec) {
							}
						}
						*/
...
// BUG - Hurry Assist - start
	...
	{
		if (GC.getDefineINT("CVGAMETEXT_HURRY_ALERT_MORE_THAN_ONCE") == 1) {//mediv01 是否能够多次提醒，如果不是1转入老代码 //似乎代码无作用，建议暂时使用0
			...
			for (iI = 0; iI < GC.getNumHurryInfos(); iI++)
			{
				if (pCity->canHurry((HurryTypes)iI))
				{
					if (bFirstHurry==0)
					{
						...
						bFirstHurry = GC.getDefineINT("CVGAMETEXT_HURRY_ALERT_INTERVAL");//mediv01 每几个回合提醒一次
					}
					else {
						bFirstHurry -= 1;
					}
...
//mediv01 科技树帮助文字
{
	...
	if (GC.getDefineINT("CVTECH_SHOW_TECH_DISCOVERY2") == 1) {
		if (bTechFirst) {
			...
			szBuffer.append(NEWLINE);
			szBuffer.append(CvWString::format(SETCOLR L"下列文明发现了这个科技：" ENDCOLR, TEXT_COLOR("COLOR_PLAYER_CYAN")));
			int show_num = 0;
			for (int iI = 0; iI < MAX_CIV_PLAYERS; iI++)//包含独立城邦
			{
				bool CivHasTech= GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHasTech(eTech);
				
				if (CivHasTech &&show_num< GC.getDefineINT("CVTECH_SHOW_TECH_DISCOVERY2_MAX")) {
					bool isalive = GET_PLAYER((PlayerTypes)iI).isAlive();
					if (isalive) {
						show_num += 1;
						szBuffer.append(CvWString::format(SETCOLR L" %s" ENDCOLR, TEXT_COLOR("COLOR_PLAYER_GREEN"), GET_PLAYER((PlayerTypes)iI).getCivilizationDescription()));
	...
	if (GC.getDefineINT("CVTECH_SHOW_TECH_DISCOVERY3") == 1) {
		if (bTechFirst) {
			...
			int show_num2 = 0;
			for (int iI = 0; iI < NUM_MAJOR_PLAYERS + 2; iI++)//包含独立城邦
			{
				bool CivHasTech = GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHasTech(eTech);
				
				if (!CivHasTech && show_num2 < GC.getDefineINT("CVTECH_SHOW_TECH_DISCOVERY3_MAX") && GET_PLAYER((PlayerTypes)iI).isAlive() ){
					show_num2 += 1;
					szBuffer1.append(CvWString::format(SETCOLR L" %s" ENDCOLR, TEXT_COLOR("COLOR_PLAYER_GREEN"), GET_PLAYER((PlayerTypes)iI).getCivilizationDescription()));
				}
				else {
					//break;
				}
	...
	if (GC.getGameINLINE().getActivePlayer() != NO_PLAYER)
	{
		if (GET_PLAYER(GC.getGameINLINE().getActivePlayer()).canResearch(eTech)) //mediv01 显示伟人点科技  || GC.getDefineINT("CVTECH_SHOW_TECH_DISCOVERY4") == 1
		{

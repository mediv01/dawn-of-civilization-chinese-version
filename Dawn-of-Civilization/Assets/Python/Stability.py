...
def checkTurn(iGameTurn):
    # 查年代和回合的代码，已经禁用
    #	for i in range(-3000,2021,1):
    #		f=open("z:\\test.csv",'a')
    #		f.write(str(i)+','+str(getTurnForYear(i))+', \n')
    #		f.close
    #utils.log('Stability Check Turn Start')
    ...
    if iGameTurn % utils.getTurns(5) == 0:
        ...
        for iPlayer in range(iNumPlayers):
            if utils.getHumanID() == iPlayer and gc.getDefineINT(
                    "STABILITY_PY_FOR_CHECK_STABILITY_FOR_HUMAN") == 1:  # mediv01 强制检查稳定度参数
                checkStability(iPlayer)
                pass
            if utils.getHumanID() != iPlayer and gc.getDefineINT(
                    "STABILITY_PY_FOR_CHECK_STABILITY_FOR_AI") == 1:  # mediv01 强制检查稳定度参数
                checkStability(iPlayer)
                pass
    ...
    #utils.log('Stability Check Turn End')


def endTurn(iPlayer):
    #utils.logwithid(iPlayer,'Stability Ending Check Turn Start')
    lSecedingCities = data.getSecedingCities(iPlayer)

    if lSecedingCities:
        secedeCities(iPlayer, lSecedingCities)
        data.setSecedingCities(iPlayer, [])
    #utils.logwithid(iPlayer,'Stability Ending Check Turn End')
...
def triggerCollapse(iPlayer):
    ...
    if utils.getHumanID() != iPlayer:  #
        if gc.getGame().getGameTurnYear() < tFall[iPlayer]:
            if len(utils.getOwnedCoreCities(iPlayer)) <= len(utils.getCityList(iPlayer)) and len(
                    utils.getOwnedCoreCities(iPlayer)) > 0:
    ...
    #	if utils.getHumanID() == iPlayer and gc.getGame().getHandicapType()<=2: #人类玩家也是如此
    #		collapseToCore(iPlayer)
    # 		return
...
def scheduleCollapse(iPlayer):
    ...
    filePath = BugPath.join(BugPath.getRootDir(), 'Saves', 'single', 'collapses',
                            '%s Collapse %d %s (turn %d) %s.CivBeyondSwordSave' % (
                            gc.getPlayer(iPlayer).getCivilizationAdjective(0), abs(gc.getGame().getGameTurnYear()),
                            epoch, gc.getGame().getGameTurn(), date.today()))
...
def setStabilityLevel(iPlayer, iStabilityLevel):
    ...
    if iStabilityLevel == iStabilityCollapsing:
        CyInterface().addMessage(iPlayer, False, iDuration,
                                 localText.getText("TXT_KEY_STABILITY_COLLAPSING_WARNING", ()), "", 0, "",
...
def isImmune_War(iPlayer):
    ...
    if iGameTurn - getTurnForYear(tBirth[iPlayer]) < calculate_war_immune():
        return True
...
def isImmune(iPlayer):
    ...
    # immune right after scenario start
    if iGameTurn - utils.getScenarioStartTurn() < calculate_stability_immune_after_Scenario_Start():
        return True

    # immune right after birth
    if iGameTurn - getTurnForYear(tBirth[iPlayer]) < calculate_stability_immune_after_birth():
        return True

    # immune right after resurrection
    if iGameTurn - pPlayer.getLatestRebellionTurn() < calculate_stability_immune_after_resurrection():
        return True
...
def determineStabilityLevel(iCurrentLevel, iStability, bFall=False):
    ...
    # was 10
    #	utils.showPopup(7622, str(bFall), \
    #		       str(' iCurrentLevel ')+str(iCurrentLevel)+str(' and stability ')+str(iStability)+str(' and iThreshold ')+str(iThreshold), \
    #		       (CyTranslator().getText("TXT_KEY_POPUP_YES", ()), \
    #			CyTranslator().getText("TXT_KEY_POPUP_NO", ())))

    #	utils.show('&#31283;&#23450;&#24230;&#20026;'+str(iStability))
...
def checkStability(iPlayer, bPositive=False, iMaster=-1):
    ...
    if(gc.getDefineINT("PYTHON_LOG_ON_STABILITY") == 1):
    ...
    if iNewStabilityLevel > iStabilityLevel:
        ...
        if iNewStabilityLevel == iStabilityCollapsing:
            # mediv01
            if (gc.getDefineINT("PYTHON_USE_ADVANCE_ALERT") == 1):  # 增加提示信息参数控制
                civname = gc.getPlayer(iPlayer).getCivilizationShortDescription(0)
                #tem_civname = CyTranslator().getText(str(civname), ())
...
def getPossibleMinors(iPlayer):
    if gc.getGame().countKnownTechNumTeams(iNationalism) == 0 and iPlayer in [iMaya, iAztecs, iInca, iMali, iEthiopia,
                                                                              iCongo]:
        return [iNative]
...
def secedeCities(iPlayer, lCities, bRazeMinorCities=False):
    ...
    for city in lCities:
        if bRazeMinorCities:
            ...
            bNotJerusalem = (not (city.getX() == 73 and city.getY() == 38)) #mediv01 地图硬编码
            ...
            if iPlayer == iHarappa and utils.getHumanID() != iPlayer:
                if (gc.getDefineINT("PYTHON_DISABLE_HARAPPA_DESTROY_CITY") == 1): #mediv01
                    pass
                else:
                    lRemovedCities.append(city)
                if (gc.getDefineINT("PYTHON_LOG_ON_MAIN_RISE_AND_FALL") == 1):
                    utils.logwithid(iPlayer, 'iHarappa lRemovedCities append: ( ' + str(city.getX()) + ',' + str(city.getY()) + ' )')
    ...
    for city in lCededCities:
        ...
        if gc.getDefineINT("STABILITY_PY_ONLY_COLLAPSE_TO_INDEPENDENT") == 0:
            iClaim = -1
            for iLoopPlayer in range(iNumPlayers):
                ...
                if cityPlot.isCore(iLoopPlayer) and gc.getPlayer(iLoopPlayer).isAlive():
                    iClaim = iLoopPlayer
                    utils.debugTextPopup('Secede ' + gc.getPlayer(iPlayer).getCivilizationAdjective(
                        0) + ' ' + city.getName() + ' to ' + gc.getPlayer(iClaim).getCivilizationShortDescription(
            ...
            if iClaim == -1:
                ...
                if cityPlot.getSettlerValue(iOriginalOwner) >= 90 and not cityPlot.isCore(
                        iPlayer) and not cityPlot in Areas.getBirthArea(iPlayer) and gc.getPlayer(
                        iOriginalOwner).isAlive() and iOriginalOwner != iPlayer and utils.getHumanID() != iOriginalOwner:
                    if iOriginalOwner < iNumPlayers and iGameTurnYear < tFall[iOriginalOwner]:
                        # cities lost too long ago don't return
                        if city.getGameTurnPlayerLost(iOriginalOwner) >= gc.getGame().getGameTurn() - utils.getTurns(
                                25):
                            iClaim = iOriginalOwner
                            utils.debugTextPopup('Secede ' + gc.getPlayer(iPlayer).getCivilizationAdjective(
                                0) + ' ' + city.getName() + ' to ' + gc.getPlayer(
                                iClaim).getCivilizationShortDescription(0) + '.\nReason: original owner.')
            ...
            if iClaim == -1:
                ...
                for iLoopPlayer in range(iNumPlayers):
                    ...
                    if pLoopPlayer.isAlive() and tPlayer.isAtWar(
                            iLoopPlayer) and utils.getHumanID() != iLoopPlayer and iGameTurnYear < tFall[iLoopPlayer]:
                        if pLoopPlayer.getWarValue(city.getX(), city.getY()) >= 8 and gc.getTeam(
                                iLoopPlayer).AI_getWarSuccess(iPlayer) > tPlayer.AI_getWarSuccess(iLoopPlayer):
                            # another enemy with closer city: don't claim the city
                            closestCity = gc.getMap().findCity(city.getX(), city.getY(), PlayerTypes.NO_PLAYER,
                                                               TeamTypes.NO_TEAM, True, False, TeamTypes.NO_TEAM,
                                                               DirectionTypes.NO_DIRECTION, city)
                            if closestCity.getOwner() != iLoopPlayer and tPlayer.isAtWar(
                                closestCity.getOwner()): continue
                            iClaim = iLoopPlayer
                            utils.debugTextPopup('Secede ' + gc.getPlayer(iPlayer).getCivilizationAdjective(
                                0) + ' ' + city.getName() + ' to ' + gc.getPlayer(
                                iClaim).getCivilizationShortDescription(0) + '.\nReason: war target.')
            ...
            for iLoopPlayer in range(iNumPlayers):
                ...
                if iPlayer == iArabia and iLoopPlayer == iEgypt and (not (gc.getDefineINT("STABILITY_EGYPT_CAN_RESPAWN_FROM_ARABIA")==1)): continue
                ...
                if tCityPlot in Areas.getRespawnArea(iLoopPlayer):
                    ...
                    if bPossible:
                        ...
                        utils.debugTextPopup(gc.getPlayer(iPlayer).getCivilizationAdjective(
                            0) + ' ' + city.getName() + ' is part of the ' + gc.getPlayer(
                            iLoopPlayer).getCivilizationAdjective(0) + ' resurrection.')
                        break
    ...
    if not bComplete:
        if gc.getPlayer(utils.getHumanID()).canContact(iPlayer):
            CyInterface().addMessage(utils.getHumanID(), False, iDuration,
                                     localText.getText("TXT_KEY_STABILITY_CITIES_SECEDED", (
                                     gc.getPlayer(iPlayer).getCivilizationDescription(0), len(lCededCities))), "", 0,
                                     "", ColorTypes(iWhite), -1, -1, True, True)
    ...
    for iResurrectionPlayer in dPossibleResurrections:
        ...
        resurrectionFromCollapse(iResurrectionPlayer, dPossibleResurrections[iResurrectionPlayer])  # mediv01
...
def secedeCity(city, iNewOwner, bRelocate):
    ...
    if city.getOwner() == utils.getHumanID():
        if iNewOwner in [iIndependent, iIndependent2, iNative, iBarbarian]:
            ...
            CyInterface().addMessage(city.getOwner(), False, iDuration, sText, "", 0, "", ColorTypes(iRed), -1, -1,
                                     True, True)
        else:
            sText = localText.getText("TXT_KEY_STABILITY_CITY_CHANGED_OWNER",
                                      (sName, gc.getPlayer(iNewOwner).getCivilizationAdjective(0)))
...
def completeCollapse(iPlayer):
    ...
    sText = localText.getText("TXT_KEY_STABILITY_COMPLETE_COLLAPSE",
                              (gc.getPlayer(iPlayer).getCivilizationAdjective(0),))
    CyInterface().addMessage(utils.getHumanID(), False, iDuration, sText, "", 0, "", ColorTypes(iWhite), -1, -1, True,
                             True)
...
def calculateStability(iPlayer):
    ...
    for city in utils.getCityList(iPlayer):
        ...
        if plot.isCore(iPlayer):
            ...
            # 殖民体系的崩溃
            if (gc.getDefineINT("STABILITY_PY_COLONY_COLLAPSE") == 1):
                if utils.getHumanID() != iPlayer and iPlayer == iChina and gc.getGame().getGameTurn() >= getTurnForYear(
                    1840) and gc.getGame().getGameTurn() <= getTurnForYear(1950): iModifier += 10

                if utils.getHumanID() != iPlayer and iPlayer == iRussia and gc.getGame().getGameTurn() >= getTurnForYear(
                    1840) and gc.getGame().getGameTurn() <= getTurnForYear(1924): iModifier += 10

                if utils.getHumanID() != iPlayer and iPlayer == iEngland and gc.getGame().getGameTurn() >= getTurnForYear(
                    1950): iModifier += 10

                if utils.getHumanID() != iPlayer and iPlayer == iFrance and gc.getGame().getGameTurn() >= getTurnForYear(
                    1950): iModifier += 10

                if utils.getHumanID() != iPlayer and iPlayer == iGermany and gc.getGame().getGameTurn() >= getTurnForYear(
                    1950): iModifier += 10

                if utils.getHumanID() != iPlayer and iPlayer == iPortugal and gc.getGame().getGameTurn() >= getTurnForYear(
                    1840): iModifier += 10

                if utils.getHumanID() != iPlayer and iPlayer == iSpain and gc.getGame().getGameTurn() >= getTurnForYear(
                    1840): iModifier += 10
            ...
            if (gc.getDefineINT("PYTHON_UHV_HELPER") == 1):
                ###奥斯曼军国主义加成
                if iPlayer == iOttomans and gc.getGame().getGameTurn() <= getTurnForYear(1800) and city.hasBuilding(
                    utils.getUniqueBuilding(iPlayer, iBarracks)): iModifier -= 1

                # 日本军国主义加成
                if utils.getHumanID() == iPlayer and iPlayer == iJapan and bTotalitarianism and gc.getGame().getGameTurn() >= getTurnForYear(
                        1900):
                    iModifier = -1

                # 罗马早期加成
                if iPlayer == iRome and gc.getGame().getGameTurn() <= getTurnForYear(330):
                    iModifier = -1

                # 迦太基早期加成
                if iPlayer == iCarthage and gc.getGame().getGameTurn() <= getTurnForYear(200):
                    iModifier = -1
    ...
    if iPeripheryExcess > 0:
        ...
        utils.debugTextPopup(
            'Expansion rating: ' + pPlayer.getCivilizationShortDescription(0) + '\nCore population: ' + str(
                iCorePopulation) + '\nPeriphery population: ' + str(
                iPeripheryPopulation) + '\nExpansion stability: ' + str(iCorePeripheryStability))
    ...
    iCivicCombinationStability = getCivicStability(iPlayer, civics) + 0

    imodifier_sta = max(gc.getDefineINT("STABILITY_PY_AI_STABILITY_BONUS"), 1)
    if utils.getHumanID() != iPlayer and iCivicCombinationStability < 0: iCivicCombinationStability /= imodifier_sta
    # AI惩罚减半参数控制

    lParameters[iParameterCivicCombinations] = iCivicCombinationStability

    iCivicEraTechStability = 0
    #	if bFall: iCivicEraTechStability -= 5

    # 修改特定文明特定时期的稳定度
    if (gc.getDefineINT("STABILITY_PY_ERA_STABILITY_FOR_BIG_COUNTRY") == 1):
        if iPlayer == iChina:
        ...
        if iPlayer == iRussia:
            if gc.getGame().getGameTurn() >= getTurnForYear(1991) and gc.getGame().getGameTurn() <= getTurnForYear(
                    2020):  # 苏联解体
                iCivicEraTechStability -= 5
            if gc.getGame().getGameTurn() >= getTurnForYear(1860) and gc.getGame().getGameTurn() <= getTurnForYear(
                    1925):  # 沙俄末期到苏联初期
                iCivicEraTechStability -= 5

        if iPlayer == iAmerica:
            if gc.getGame().getGameTurn() >= getTurnForYear(1776) and gc.getGame().getGameTurn() <= getTurnForYear(
                    1865):  # 建国至美国内战
                iCivicEraTechStability -= 5

        if iPlayer == iEngland:
            if gc.getGame().getGameTurn() >= getTurnForYear(1914) and gc.getGame().getGameTurn() <= getTurnForYear(
                    2020):  # 英国一战后衰落
                iCivicEraTechStability -= 5

        if iPlayer == iFrance:
            if gc.getGame().getGameTurn() >= getTurnForYear(1870) and gc.getGame().getGameTurn() <= getTurnForYear(
                    2020):  # 法国普法战争后衰落
                iCivicEraTechStability -= 5
            if gc.getGame().getGameTurn() >= getTurnForYear(1938) and gc.getGame().getGameTurn() <= getTurnForYear(
                    1945):  # 法国二战投降！
                iCivicEraTechStability -= 10
            if gc.getGame().getGameTurn() >= getTurnForYear(1789) and gc.getGame().getGameTurn() <= getTurnForYear(
                    1830):  # 法国大革命
                iCivicEraTechStability -= 5
            if gc.getGame().getGameTurn() >= getTurnForYear(1337) and gc.getGame().getGameTurn() <= getTurnForYear(
                    1453):  # 英法百年战争
                iCivicEraTechStability -= 5

        if iPlayer == iJapan:
            if gc.getGame().getGameTurn() >= getTurnForYear(1467) and gc.getGame().getGameTurn() <= getTurnForYear(
                    1615):  # 日本战国时期
                iCivicEraTechStability -= 5
            if gc.getGame().getGameTurn() >= getTurnForYear(1780) and gc.getGame().getGameTurn() <= getTurnForYear(
                    1867):  # 幕府晚期
                iCivicEraTechStability -= 5
            if gc.getGame().getGameTurn() >= getTurnForYear(1937) and gc.getGame().getGameTurn() <= getTurnForYear(
                    1960):  # 二战战败
                iCivicEraTechStability -= 5
            if gc.getGame().getGameTurn() >= getTurnForYear(1990) and gc.getGame().getGameTurn() <= getTurnForYear(
                    2020):  # 现代日本泡沫经济破裂
                iCivicEraTechStability -= 5

        if iPlayer == iIndia or iPlayer == iMughals:
            if gc.getGame().getGameTurn() >= getTurnForYear(1858) and gc.getGame().getGameTurn() <= getTurnForYear(
                    1947):  # t印度殖民地时期
                iCivicEraTechStability -= 5
        ...
        if iPlayer == iGermany or iPlayer == iHolyRome:
            if gc.getGame().getGameTurn() >= getTurnForYear(1914) and gc.getGame().getGameTurn() <= getTurnForYear(
                    1933):  # 德国一战战败
                iCivicEraTechStability -= 5
            if gc.getGame().getGameTurn() >= getTurnForYear(1945) and gc.getGame().getGameTurn() <= getTurnForYear(
                    1990):  # 德国二战战败
                iCivicEraTechStability -= 5
        ...
        if iPlayer == iSpain:
            if gc.getGame().getGameTurn() >= getTurnForYear(1588) and gc.getGame().getGameTurn() <= getTurnForYear(
                    1950):  # 无敌舰队的覆灭
                iCivicEraTechStability -= 5

        if iPlayer == iPortugal:
            if gc.getGame().getGameTurn() >= getTurnForYear(1650) and gc.getGame().getGameTurn() <= getTurnForYear(
                    1950):  # 葡萄牙的衰落
                iCivicEraTechStability -= 5

        if iPlayer == iNetherlands:
            if gc.getGame().getGameTurn() >= getTurnForYear(1784) and gc.getGame().getGameTurn() <= getTurnForYear(
                    1950):  # 英荷战争后衰落
                iCivicEraTechStability -= 5

    if (gc.getDefineINT("STABILITY_PY_AI_COLLAPSE_TO_CORE_REGULARLY") == 1):
        # mediv01 AI大帝国的崩溃，特定时期崩溃
        if iPlayer == iGreece and gc.getGame().getGameTurn() >= getTurnForYear(0) and utils.getHumanID() != iPlayer:
            iCivicEraTechStability -= 20
            pass
        if iPlayer == iRome and gc.getGame().getGameTurn() >= getTurnForYear(500) and utils.getHumanID() != iPlayer:
            iCivicEraTechStability -= 20
            pass
        if iPlayer == iChina and gc.getGame().getGameTurn() >= getTurnForYear(
                180) and gc.getGame().getGameTurn() <= getTurnForYear(550) and utils.getHumanID() != iPlayer:
            iCivicEraTechStability -= 20
            pass
        if iPlayer == iChina and gc.getGame().getGameTurn() >= getTurnForYear(
                900) and gc.getGame().getGameTurn() <= getTurnForYear(1400) and utils.getHumanID() != iPlayer:
            iCivicEraTechStability -= 20
            pass
        if iPlayer == iOttomans and gc.getGame().getGameTurn() >= getTurnForYear(
                1800) and gc.getGame().getGameTurn() <= getTurnForYear(2020) and utils.getHumanID() != iPlayer:
            iCivicEraTechStability -= 20

        if iPlayer == iIndia and gc.getGame().getGameTurn() >= getTurnForYear(
                1500) and gc.getGame().getGameTurn() <= getTurnForYear(2020) and utils.getHumanID() != iPlayer:
            iCivicEraTechStability -= 20

        if iPlayer == iMughals and gc.getGame().getGameTurn() >= getTurnForYear(
                1800) and gc.getGame().getGameTurn() <= getTurnForYear(2020) and utils.getHumanID() != iPlayer:
            iCivicEraTechStability -= 20


    if (gc.getDefineINT("STABILITY_PY_BONUS_FOR_SOCIALIST_COUNTRY") == 1):  # 社会主义国家的稳定度选项
    ...
    if (gc.getDefineINT("STABILITY_PY_HANDICAP_BONUS") == 1):
    ...
    if iCivicReligion == iDeification:
        # 古典时代或更早期的神化：+2   古典时代后的神化：古典时代后的每个时代-2
    ...
    # 具有印度教宗教的种姓制度：+3
        ...
        if iCivicLegitimacy == iMeritocracy: iCivicEraTechStability += 5
        if iCivicTerritory == iIsolationism: iCivicEraTechStability += 3
    # 儒家国教的精英统治主义：+5     孤立主义+3
    ...
    if utils.getHumanID() != iPlayer and iCivicEraTechStability < 0: iCivicEraTechStability /= 1
    # AI惩罚减半
    ...
    iRelationWeaker = 0
    iNegativeWeaker = max(0, count(lWeakerAttitudes, lambda x: x < -1) - count(lWeakerAttitudes, lambda
        x: x >= 3 + iAttitudeThresholdModifier))
    ...
    iRelationEqual = sum(
        sign(iAttitude) * min(25, abs(iAttitude) / 5) for iAttitude in lEqualAttitudes if abs(iAttitude) > 2)
    ...
    # 将外交影响降至1/5的水平，参数控制
    # iVassalStability=round(iVassalStability/5)
    # iDefensivePactStability =round(iDefensivePactStability /5)
    # iRelationStability=round(iRelationStability/5)
    # iNationhoodStability =round(iNationhoodStability /5)
    # iTheocracyStability=round(iTheocracyStability/5)
    # iMultilateralismStability=round(iMultilateralismStability/5)
    modif_dipo = max(gc.getDefineINT("STABILITY_PY_DIPO_STABILITY_MODIFIER"), 1)
    ...
    for iEnemy in range(iNumPlayers):
        ...
        if pEnemy.isAlive() and tPlayer.isAtWar(iEnemy):
            ...
            utils.debugTextPopup_WarWeariness(
                pPlayer.getCivilizationAdjective(0) + ' war against ' + pEnemy.getCivilizationShortDescription(
                    0) + '\nWar Success Stability: ' + str(iTempWarSuccessStability) + '\nWar Weariness: ' + str(
                    iTempWarWearinessStability))
    ...
    if (gc.getDefineINT("STABILITY_PY_NOT_CHECK_STABILITY") == 1):  # 关闭稳定度计算的参数
        iStability = 0

    return iStability, [iExpansionStability, iEconomyStability, iDomesticStability, iForeignStability,
                        iMilitaryStability], lParameters
...
def getCivicStability(iPlayer, lCivics):
    ...
    #	f=open('Z:\\'+str(iPlayer)+'.csv','a')
    #	f.write(str(iPlayer)+','+str(iStability))
    #	f.close
    return iStability
...
def other(lCombination, *civics):
    ...
    lCivics = [iCivic for iCivic in range(iNumCivics) if
               gc.getCivicInfo(iCivic).getCivicOptionType() == iCategory and iCivic not in civics]
    return set(lCivics) & lCombination
...
def updateHappinessTrend(iPlayer):
    ...
    for city in utils.getCityList(iPlayer):
        ...
        if city.isWeLoveTheKingDay() or (
                iPopulation >= iAveragePopulation and iHappiness - iUnhappiness >= iAveragePopulation / 4):
            iHappyCities += 1
...
def calculateCommerceRank(iPlayer, iTurn):
    lCommerceValues = utils.getSortedList([i for i in range(iNumPlayers)],
                                          lambda x: gc.getPlayer(x).getEconomyHistory(iTurn), True)
    return lCommerceValues.index(iPlayer)


def calculatePowerRank(iPlayer, iTurn):
    lPowerValues = utils.getSortedList([i for i in range(iNumPlayers)],
                                       lambda x: gc.getPlayer(x).getPowerHistory(iTurn), True)
...
def calculateRankedAttitudes(iPlayer, lContacts):
    ...
    lEqual = [calculateAttitude(iLoopPlayer, iPlayer) for iLoopPlayer in lContacts[iLeft:iRight] if
              iLoopPlayer != iPlayer]
    ...
    for iLoopCiv in utils.getSortedList(lPossibleResurrections, lambda x: data.players[x].iLastTurnAlive):
        ...
        for city in utils.getAreaCities(Areas.getRespawnArea(iLoopCiv)):
            if city.getOwner() not in [iIndependent, iIndependent2, iNative, iBarbarian]:
                if (gc.getDefineINT("PYTHON_LOG_ON_MAIN_RISE_AND_FALL") == 1):
                    utils.logwithid(iLoopCiv, ':city cannot resurrection: city not in minor, now in '+str(gc.getPlayer(city.getOwner()).getCivilizationShortDescription(0)))
                break
        else:
...
def doResurrection(iPlayer, lCityList, bAskFlip=True):
    ...
    if(gc.getDefineINT("STABILITY_RESURRECTION_ARMY_SIZE")>0):
        iNumCities=min(gc.getDefineINT("STABILITY_RESURRECTION_ARMY_SIZE"),iNumCities)
        iArmySize=min(gc.getDefineINT("STABILITY_RESURRECTION_ARMY_SIZE"),iArmySize)
    ...
    for city in lCityList:      #计算复活的城市
        iOwner = city.getOwner()
        pOwner = gc.getPlayer(iOwner)
        if (gc.getDefineINT("STABILITY_RESURRECTION_NOT_FLIP_HUMAN_CITY")>0):
        ...
        for iBuilding in range(iNumBuildings):
            if pPlayer.canConstruct(iBuilding, True, False, False) and newCity.canConstruct(iBuilding, True, False,
                                                                                            False) and pPlayer.getCurrentEra() >= gc.getBuildingInfo(
                    iBuilding).getFreeStartEra() and not utils.isUniqueBuilding(iBuilding) and gc.getBuildingInfo(
                    iBuilding).getPrereqReligion() == -1:
    ...
    CyInterface().addMessage(iHuman, True, iDuration, CyTranslator().getText("TXT_KEY_INDEPENDENCE_TEXT",
                                                                             (pPlayer.getCivilizationAdjectiveKey(),)),
                             "", 0, "", ColorTypes(iGreen), -1, -1, True, True)
    ...
    return
def getResurrectionTechs_old_20210221(iPlayer): #LEO版定义的复活科技，我觉得设定不合理 20210221
    pPlayer = gc.getPlayer(iPlayer)
    lTechList = []
    lSourceCivs = []
...
def getResurrectionTechs(iPlayer):
    ...
    if(gc.getDefineINT("STABILITY_RESURRECTION_TECH_WITH_WORLD_IMPORTANT_CIV")>0):
        ImportCiv=[iRome,iRussia,iFrance,iChina,iEngland,iAmerica,iByzantium,iArabia,iSpain,iOttomans]
        for iPeer in ImportCiv:
            if iPeer != iPlayer and iPeer not in lSourceCivs and gc.getPlayer(iPeer).isAlive():
                if (len(lSourceCivs) <= gc.getDefineINT("STABILITY_RESURRECTION_TECH_WITH_WORLD_IMPORTANT_CIV")):
                    lSourceCivs.append(iPeer)
...
def relocateCapital(iPlayer, bResurrection=False):
    ...
    if plot.isCity() and plot.getPlotCity().getOwner() == iPlayer:
        ...
        newCapital = utils.getHighestEntry(utils.getCityList(iPlayer), lambda x: max(0, utils.getTurns(
            500) - x.getGameTurnFounded()) + x.getPopulation() * 5)
...
def setStateReligion(iCiv):
    ...
    for city in lCities:
        for iReligion in range(iNumReligions):
            if gc.getReligionInfo(iReligion).isLocal() and city.plot().getSpreadFactor(
                iReligion) != RegionSpreadTypes.REGION_SPREAD_CORE: continue
...
def rebellionPopup(iRebelCiv):
    utils.showPopup(7622, CyTranslator().getText("TXT_KEY_REBELLION_TITLE", ()), \
                    CyTranslator().getText("TXT_KEY_REBELLION_TEXT",
                                           (gc.getPlayer(iRebelCiv).getCivilizationAdjectiveKey(),)), \
                    (CyTranslator().getText("TXT_KEY_POPUP_YES", ()), \
                     CyTranslator().getText("TXT_KEY_POPUP_NO", ())))
...
class Civics:
    ...
    def no(self, civic):
        if gc.getCivicInfo(civic).getCivicOptionType() not in [gc.getCivicInfo(i).getCivicOptionType() for i in
                                                               self.activeCivics]: return False

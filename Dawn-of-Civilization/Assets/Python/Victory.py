...
tSouthAsiaTL = (88, 24)
tSouthAsiaBR = (110, 38)
lSouthAsianCivs = [iIndia, iTamils, iIndonesia, iKhmer, iMughals, iThailand, iChina]
...
def checkTurn(iGameTurn, iPlayer):
    ...
    if iPlayer == iEgypt:
        ...
        if isPossible(iTurks, 0):
            if getLandPercent(iTurks) >= 6.995 and data.iTurkicPillages >= 40:
                win(iTurks, 0)
        ...
        if isPossible(iTurks, 1):
            if isConnectedByTradeRoute(iTurks, utils.getPlotList(tChinaTL, tChinaBR), lMediterraneanPorts) and pTurks.countCorporations(iSilkRoute) >= 7:
                win(iTurks, 1)
        ...
        if iGameTurn == getTurnForYear(1100):
            bEgypt = isControlledOrVassalizedGroup([iArabia, iEgypt], Areas.getCoreArea(iEgypt, False))
            bMaghreb = isControlledOrVassalizedGroup([iArabia, iMoors], utils.getPlotList(tCarthageTL, tCarthageBR))
            ...
            bSpain = isControlledOrVassalizedGroup([iArabia, iMoors], Areas.getNormalArea(iSpain, False))
        ...
        if isPossible(iArabia, 2):
            bJerusalemInEnermy = not isControlled(iArabia, utils.getPlotList((72, 37), (74, 39)))
            bMeccaInEnermy = not isControlled(iArabia, utils.getPlotList((74, 32), (76, 34)))
            # bMedinaInEnermy=not isControlled(iArabia, utils.getPlotList(tCarthageTL, tCarthageBR))
            bCityInEnermy = (bJerusalemInEnermy or bMeccaInEnermy)
            if iGameTurn >= getTurnForYear(650) and bCityInEnermy:
                lose(iArabia, 2)
            if gc.getGame().calculateReligionPercent(iIslam) >= 20:
                win(iArabia, 2)
        ...
        if isPossible(iTibet, 2):
            if countCitySpecialists(iTibet, Areas.getCapital(iPlayer), iSpecialistGreatProphet) >= 5:
                win(iTibet, 2)
            elif countCitySpecialists(iTibet, (96, 42), iSpecialistGreatProphet) >= 5:
                win(iTibet, 2)
        ...
        if isPossible(iIndonesia, 2):
            if getPopulationPercent(iIndonesia) >= 16.0:
                win(iIndonesia, 2)
        ...
        if (isPossible(iFrance, 0)):
            iCulture = max(getCityCulture(iFrance, (55, 50)), getCityCulture(iFrance, (55, 51)))
            if iCulture >= utils.getTurns(VIC_FRANCE_CULTURE):
                win(iFrance, 0)
            else:
                # lose(iFrance, 0)
                pass
        ...
        if (isPossible(iFrance, 1)):
            ...
            [fEurope, fNorthAmerica] = getLandPercentInEurope(iFrance)
        ...
        if (isPossible(iFrance, 2)):
            ...
            if (bNotreDame and bVersailles and bLouvre and bEiffelTower and bMetropolitain):
                win(iFrance, 2)
        ...
        # second goal: control golden one, iMachuPicchu and 4 gold or silver resource by 1700AD
        if isPossible(iInca, 1):
            bMachuPicchu = (getNumBuildings(iInca, iMachuPicchu) >= 1)
            bGoleden_One = isControlledTile(iInca, [Areas_Goleden_One])
            bResouce = countAcquiredResources(iInca, [iGold, iSilver]) >= 4
            if bMachuPicchu and bGoleden_One and bResouce:
                win(iInca, 1)
                pass

        if isPossible(iInca, 2):
            bSouthPacific = isControlledTile(iInca, [[21, 16]])
            tExceptions = list(set(lAndeanCoast).union(set([(26, 29), (27, 22), (28, 22), (30, 20), (31, 19)])))
            lWestAmazon = utils.getPlotList((23, 18), (34, 28), tExceptions)
            bWestAmazon = getNumCitiesInArea(iInca, lWestAmazon) >= 3
            bLaPlata = isControlledTile(iInca, [[34, 12]])
            if bSouthPacific and bWestAmazon and bLaPlata:
                win(iInca, 2)
                pass
            if iGameTurn == getTurnForYear(1700):
                expire(iInca, 2)
        ...
        if isPossible(iCongo, 0):
            if getApostolicVotePercent(iCongo) >= 10.0:
                win(iCongo, 0)
        ...
        if isPossible(iAmerica, 0):
            bAmerican= isControlledOrVassalized(iAmerica, utils.getPlotList([11, 43], [32, 50], []))
            lLosAngles = utils.getPlotList((11, 47), (11, 47), [])
            bLosAngles = isControlledTile(iAmerica, lLosAngles)
            lSeatle = utils.getPlotList((11, 50), (11, 50), [])
            bSeatle = isControlledTile(iAmerica, lSeatle)
            lPlots=utils.getPlotList((11, 44), (21, 52), [])
            iNumCity=countCitiesinRegion(iAmerica,lPlots)
            bCity=(iNumCity>=5)

            bRailway1 = isConnectedByRailroad_withoutCulture(iAmerica, [30,48], [(11, 47)])
            bRailway2 = isConnectedByRailroad_withoutCulture(iAmerica, [30,48], [(11,51)])
            bRailway3 = isConnectedByRailroad_withoutCulture(iAmerica, [30,48], [(27,46)])
            bRailway = bRailway1 and bRailway2 and bRailway3
            if(bAmerican and bLosAngles and bSeatle and bRailway and bCity):
                win(iAmerica,0)
            pass
        ...
        if isPossible(iAmerica, 1):
            ...
            bWonder=bUnitedNations and bBrooklynBridge and bStatueOfLiberty and bGoldenGateBridge and bPentagon and bEmpireState and bHollyWood

            lTechList = [iFission, iComputers, iNuclearPower]
            bFission = checkTechGoal(iAmerica, [iFission])
            bComputor = checkTechGoal(iAmerica, [iComputers])
            bNuclearPower = checkTechGoal(iAmerica, [iNuclearPower])
            bTech=bFission and bComputor and bNuclearPower


            bManhattanProject = teamAmerica.getProjectCount(iManhattanProject) > 0
            bInternetProject = teamAmerica.getProjectCount(iTheInternet) > 0
            bProject= bManhattanProject and bInternetProject

            if(bWonder and bTech and bProject):
                win(iAmerica,1)
            pass
        ...
        if isPossible(iAmerica, 2):
            lTechList=[iSpaceflight,iSatellites]
            bSpaceflight=checkTechGoal(iAmerica,[iSpaceflight])
            bSatellites=checkTechGoal(iAmerica,[iSatellites])
            bTech = bSpaceflight and bSatellites

            bFIRST_SATELLITE = teamAmerica.getProjectCount(iFirstSatellite) > 0
            bManinSpace = teamAmerica.getProjectCount(iManInSpace) > 0
            bApolloProgram = teamAmerica.getProjectCount(iLunarLanding) > 0
            bISS = teamAmerica.getProjectCount(iISS) > 0
            bHubble=data.getWonderBuilder(iHubbleSpaceTelescope) == iAmerica
            bWTC=data.getWonderBuilder(iWorldTradeCenter) == iAmerica
            bWonder = bFIRST_SATELLITE and bManinSpace and bApolloProgram and bISS and bHubble and bWTC



            bAllied = calculateAlliedCommercePercent(iAmerica) >= 75.0 and calculateAlliedPowerPercent(iAmerica) >= 75.0





            if(bWonder and bTech and bAllied):
                win(iAmerica,2)
            pass
        ...
        if isPossible(iAmerica, 1):
            ...
            bPentagon = data.getWonderBuilder(iPentagon) == iAmerica
            bEmpireState = data.getWonderBuilder(iEmpireStateBuilding) == iAmerica
            bWonder = (bUnitedNations and bBrooklynBridge and bStatueOfLiberty and bPentagon and bEmpireState and bGoldenGateBridge)
...
def onPlayerSlaveTrade(iPlayer, iGold):
    ...
    if iPlayer == iCongo:
        if isPossible(iCongo, 1):
            ...
            if data.iCongoSlaveCounter >= utils.getTurns(500):
                win(iCongo, 1)
...
def checkReligiousGoal(iPlayer, iGoal):
    pPlayer = gc.getPlayer(iPlayer)
    iVictoryType = utils.getReligiousVictoryType(iPlayer)
    if (gc.getDefineINT("PYTHON_NO_URV_CHECK") == 1):
        return
        # mediv01 禁用URV
    if iVictoryType == -1:
        return -1
...
def isControlledTile(iPlayer, lPlots):
    lOwners = []
    for tPlot in lPlots:
        x, y = tPlot
        plot = gc.getMap().plot(x, y)
...
def isControlledOrVassalizedGroup(iPlayerList, lPlots):
    bControlled = False
    lOwners = []
    lValidOwners = iPlayerList
    for city in utils.getAreaCities(lPlots):
        iOwner = city.getOwner()
        if iOwner not in lOwners and iOwner < iNumPlayers:
            lOwners.append(iOwner)
    for iLoopPlayer in range(iNumPlayers):
        for iPlayer in iPlayerList:
            if gc.getTeam(gc.getPlayer(iLoopPlayer).getTeam()).isVassal(iPlayer):
                lValidOwners.append(iLoopPlayer)
    for iLoopPlayer in lValidOwners:
        if iLoopPlayer in lOwners:
...
def isConnectedByTradeRoute(iPlayer, lStarts, lTargets):
    for tStart in lStarts:
        ...
        if gc.getDefineINT("PYTHON_VIC_CONNECTED_BY_ROAD_WITH_NEW_METHOD") == 1:
            plotFunction = lambda tPlot: utils.plot(tPlot).getOwner() not in [iBarbarian, iCeltia] and (utils.plot(tPlot).isCity() or utils.plot(tPlot).getRouteType() in [iRouteRoad, iRouteRailroad, iRouteRomanRoad, iRouteHighway])
        else:
...
def countCitiesinRegion(iPlayer, lPlots):
    iCount = 0
    for city in utils.getCityList(iPlayer):
        if (city.getX(), city.getY()) in lPlots :
            iCount += 1
    return iCount
...
def getUHVHelp(iPlayer, iGoal):
    ...
    aHelp = checkUHVhelp3000(iPlayer, iGoal, aHelp)
    aHelp = checkUHVhelp600(iPlayer, iGoal, aHelp)
    aHelp = checkUHVhelp1700(iPlayer, iGoal, aHelp)
    return aHelp


def checkUHVhelp3000(iPlayer, iGoal, aHelp):
    if iPlayer == iEgypt:
        if iGoal == 0:
    ...
    # Maya goals have no stages
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(fLandPercent >= 6.995) + localText.getText("TXT_KEY_VICTORY_PERCENTAGE_WORLD_TERRITORY", (str(u"%.2f%%" % fLandPercent), str(7))))
            aHelp.append(getIcon(iPillagedImprovements >= 40) + localText.getText("TXT_KEY_VICTORY_PILLAGED_IMPROVEMENTS", (iPillagedImprovements, 40)))
        elif iGoal == 1:
            ...
            aHelp.append(getIcon(iSilkRouteCities >= 7) + localText.getText("TXT_KEY_VICTORY_CITIES_WITH_SILK_ROUTE", (iSilkRouteCities, 7)))
        elif iGoal == 2:
            iCultureLevel = 3
...
def checkUHVhelp600(iPlayer, iGoal, aHelp):
    if iPlayer == iArabia:  # mediv01 修改UHV
        if iGoal == 0:
            ...
            fReligionPercent = gc.getGame().calculateReligionPercent(iIslam)
            bJerusalemInArabia = isControlled(iArabia, utils.getPlotList((72, 37), (74, 39)))
            bMeccaInArabia = isControlled(iArabia, utils.getPlotList((74, 32), (76, 34)))
            aHelp.append(getIcon(fReligionPercent >= 20.0) + localText.getText("TXT_KEY_VICTORY_SPREAD_RELIGION_PERCENT", (gc.getReligionInfo(iIslam).getTextKey(), str(u"%.2f%%" % fReligionPercent), str(20))))
            aHelp.append(getIcon(bJerusalemInArabia) + localText.getText("TXT_KEY_VICTORY_ARABIA_UHV3_JERUSALEM", ()))
            aHelp.append(getIcon(bMeccaInArabia) + localText.getText("TXT_KEY_VICTORY_ARABIA_UHV3_MECCA", ()))
        ...
        if iGoal == 0:
            ...
            iCounter = countCitySpecialists(iTibet, Areas.getCapital(iTibet), iSpecialistGreatProphet) + countCitySpecialists(iTibet, (96, 42), iSpecialistGreatProphet)
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(popPercent >= 16.0) + localText.getText("TXT_KEY_VICTORY_PERCENTAGE_WORLD_POPULATION", (str(u"%.2f%%" % popPercent), str(16))))
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(iCulture >= utils.getTurns(VIC_FRANCE_CULTURE)) + localText.getText("TXT_KEY_VICTORY_CITY_CULTURE", ("Paris", iCulture, utils.getTurns(VIC_FRANCE_CULTURE))))
        elif iGoal == 1:
            ''' #以前版本的代码
            iEurope, iTotalEurope = countControlledTiles(iFrance, tEuropeTL, tEuropeBR, True)
            iEasternEurope, iTotalEasternEurope = countControlledTiles(iFrance, tEasternEuropeTL, tEasternEuropeBR, True)
            ...
            if (gc.getDefineINT("PYTHON_UHV_FRANCE_SHOW_OTHER_COUNTRY") > 0):
                iAllCiv = (iEgypt, iBabylonia, iHarappa, iChina, iGreece, iIndia, iCarthage, iPolynesia, iPersia, iRome,
                           ...
                           iGermany, iAmerica, iArgentina, iBrazil, iCanada)
                aHelp.append("Other Country in European:")
                lPercent = []
                for iCiv in iAllCiv:
                    [fEurope, fNorthAmerica] = getLandPercentInEurope(iCiv)
                    lPercent.append([fEurope, fNorthAmerica, iCiv])
                    pass
                lPercent.sort(key=lambda x: -(x[0] + x[1]))

                for i in range(gc.getDefineINT("PYTHON_UHV_FRANCE_SHOW_OTHER_COUNTRY")):
                    fEurope = lPercent[i][0]
                    fNorthAmerica = lPercent[i][1]
                    iCiv = lPercent[i][2]

                    aHelp.append(gc.getPlayer(iCiv).getCivilizationDescription(0) + ":  " + localText.getText("TXT_KEY_VICTORY_EUROPEAN_TERRITORY", (str(u"%.2f%%" % fEurope), str(40))) + '      ' + localText.getText("TXT_KEY_VICTORY_NORTH_AMERICAN_TERRITORY", (str(u"%.2f%%" % fNorthAmerica), str(40))))
        ...
        if iGoal == 0:
            ...
            bGoleden_One = isControlledTile(iInca, [Areas_Goleden_One])
            bResouce = countAcquiredResources(iInca, [iGold, iSilver]) >= 4
            aHelp.append(getIcon(bMachuPicchu) + localText.getText("TXT_KEY_VICTORY_INCA_MachuPicchu", ()) + ' ' + getIcon(bGoleden_One) + localText.getText("TXT_KEY_VICTORY_INCA_Goleden_One", ()) + ' ' + getIcon(bResouce) + localText.getText("TXT_KEY_VICTORY_INCA_Resouce_GOLD", (countAcquiredResources(iInca, [iGold, iSilver]), 4)))
            ...
            bLaPlata = isControlledTile(iInca, [[34, 12]])
            txt1 = getIcon(bSouthPacific) + localText.getText("TXT_KEY_VICTORY_INCA_CONTROL_SOUTH_PACIFIC", ()) + ' '
            txt2 = getIcon(bWestAmazon) + localText.getText("TXT_KEY_VICTORY_INCA_CONTROL_WestAmazon", (getNumCitiesInArea(iInca, lWestAmazon), 3)) + ' '
            txt3 = getIcon(bLaPlata) + localText.getText("TXT_KEY_VICTORY_INCA_CONTROL_LaPlata", ()) + ' '
        ...
        if iGoal == 0:
            ...
            for city in utils.getAreaCities(utils.getPlotList(tSouthAsiaTL, tSouthAsiaBR)):
                iOwner = city.getOwner()
                if iOwner < iNumPlayers and iOwner not in lSouthAsianCivs:
                    civname = gc.getPlayer(iOwner).getCivilizationShortDescription(0)
                    aHelp.append(getIcon(bSouthAsia) + str(civname))


    elif iPlayer == iCongo:
        if iGoal == 0:
            fPercent = getApostolicVotePercent(iCongo)
            aHelp.append(getIcon(fPercent >= 10.0) + localText.getText("TXT_KEY_VICTORY_APOSTOLIC_VOTE_PERCENT", (str(u"%.2f%%" % fPercent), str(15))))
        elif iGoal == 1:
            iSlaves = data.iCongoSlaveCounter
            aHelp.append(getIcon(iSlaves >= utils.getTurns(500)) + localText.getText("TXT_KEY_VICTORY_SLAVES_TRADED", (iSlaves, utils.getTurns(1000))))
...
def checkUHVhelp1700(iPlayer, iGoal, aHelp):
    if iPlayer == iGermany:
        ...
        if iGoal == 0:
            ...
            bRailway3 = isConnectedByRailroad_withoutCulture(iAmerica, [30,48], [(27,46)])
            bRailway = bRailway1 and bRailway2 and bRailway3
            txt1=getIcon(bAmerican) +utils.getText("TXT_KEY_VICTORY_AME1_TIP1_1")+getIcon(bLosAngles) +utils.getText("TXT_KEY_VICTORY_AME1_TIP1_2")+getIcon(bSeatle) +utils.getText("TXT_KEY_VICTORY_AME1_TIP1_3")
            aHelp.append(txt1)
            lPlots=utils.getPlotList((11, 44), (21, 52), [])
            iNumCity=countCitiesinRegion(iAmerica,lPlots)
            bCity=(iNumCity>=5)
            txt1=getIcon(bCity) +utils.getText('TXT_KEY_VICTORY_AME1_TIP1_5')+str(iNumCity)+' / '+str(5)
            aHelp.append(txt1)
            txt1=getIcon(bRailway) +utils.getText("TXT_KEY_VICTORY_AME1_TIP1_4")
            aHelp.append(txt1)
            '''
            ...
            aHelp.append(getIcon(bGoldenGateBridge) + localText.getText("TXT_KEY_BUILDING_GOLDEN_GATE_BRIDGE", ()) + ' ' + getIcon(bPentagon) + localText.getText("TXT_KEY_BUILDING_PENTAGON", ()) + ' ' + getIcon(bUnitedNations) + localText.getText("TXT_KEY_BUILDING_UNITED_NATIONS", ())+getIcon(bHollyWood) + localText.getText("TXT_KEY_BUILDING_HOLLYWOOD", ()))

            lTechList=[iFission,iComputers,iNuclearPower]
            bFission=checkTechGoal(iAmerica,[iFission])
            bComputor=checkTechGoal(iAmerica,[iComputers])
            bNuclearPower=checkTechGoal(iAmerica,[iNuclearPower])
            txt1=utils.getText('TXT_KEY_VICTORY_AME2_TIP1_1')+getIcon(bFission) +utils.getTechNameCn(iFission)+getIcon(bComputor) +utils.getTechNameCn(iComputers)+getIcon(bNuclearPower) +utils.getTechNameCn(iNuclearPower)
            aHelp.append(txt1)


            bManhattanProject = teamAmerica.getProjectCount(iManhattanProject) > 0
            bInternetProject = teamAmerica.getProjectCount(iTheInternet) > 0
            txt1=utils.getText('TXT_KEY_VICTORY_AME2_TIP1_2')+getIcon(bManhattanProject) + localText.getText("TXT_KEY_PROJECT_MANHATTAN_PROJECT", ()) + ' '+getIcon(bInternetProject) + localText.getText("TXT_KEY_PROJECT_THEINTERNET_PROJECT", ())
            aHelp.append(txt1)
            ...
            bSpaceflight=checkTechGoal(iAmerica,[iSpaceflight])
            bSatellites=checkTechGoal(iAmerica,[iSatellites])
            txt1=utils.getText('TXT_KEY_VICTORY_AME2_TIP1_1')+getIcon(bSpaceflight) +utils.getTechNameCn(iSpaceflight)+getIcon(bSatellites) +utils.getTechNameCn(iSatellites)

            aHelp.append(txt1)
            txt1=''+utils.getText('TXT_KEY_VICTORY_AME2_TIP1_2')
            bFIRST_SATELLITE = teamAmerica.getProjectCount(iFirstSatellite) > 0
            txt1+= getIcon(bFIRST_SATELLITE) + localText.getText("TXT_KEY_PROJECT_FIRST_SATELLITE", ())

            bManinSpace = teamAmerica.getProjectCount(iManInSpace) > 0
            txt1+= getIcon(bManinSpace) + localText.getText("TXT_KEY_PROJECT_MAN_IN_SPACE", ())

            bApolloProgram = teamAmerica.getProjectCount(iLunarLanding) > 0
            txt1+= getIcon(bApolloProgram) + localText.getText("TXT_KEY_PROJECT_LUNAR_LANDING", ())

            bISS = teamAmerica.getProjectCount(iISS) > 0
            txt1+= getIcon(bISS) + localText.getText("TXT_KEY_PROJECT_INTERNATIONAL_SPACE_STATION", ())

            bHubble=data.getWonderBuilder(iHubbleSpaceTelescope) == iAmerica
            txt1+= getIcon(bHubble) + localText.getText("TXT_KEY_BUILDING_HUBBLE_SPACE_TELESCOPE", ())

            bWTC=data.getWonderBuilder(iWorldTradeCenter) == iAmerica
            txt1+= getIcon(bWTC) + localText.getText("TXT_KEY_BUILDING_WORLD_TRADE_CENTER", ())
...
def getLandPercentInEurope(iCountryInEurope):
    if (gc.getDefineINT("PYTHON_UHV_FRANCE_CONTROL_TILES_IN_COUNTRY") == 1):
        iEurope, iTotalEurope = countControlledTiles_Alive(iCountryInEurope, tEuropeTL, tEuropeBR, True)
        iEasternEurope, iTotalEasternEurope = countControlledTiles_Alive(iCountryInEurope, tEasternEuropeTL, tEasternEuropeBR, True)
        iNorthAmerica, iTotalNorthAmerica = countControlledTiles_Alive(iCountryInEurope, tNorthAmericaTL, tNorthAmericaBR, True)
        pass
    else:

        iEurope, iTotalEurope = countControlledTiles(iCountryInEurope, tEuropeTL, tEuropeBR, True)
        iEasternEurope, iTotalEasternEurope = countControlledTiles(iCountryInEurope, tEasternEuropeTL, tEasternEuropeBR, True)
        iNorthAmerica, iTotalNorthAmerica = countControlledTiles(iCountryInEurope, tNorthAmericaTL, tNorthAmericaBR, True)
    fEurope = (iEurope + iEasternEurope) * 100.0 / (iTotalEurope + iTotalEasternEurope)
    fNorthAmerica = iNorthAmerica * 100.0 / iTotalNorthAmerica
    return [fEurope, fNorthAmerica]


def countControlledTiles_Alive(iPlayer, tTopLeft, tBottomRight, bVassals=False, lExceptions=[], bCoastalOnly=False):
    lValidOwners = [iPlayer]
    iCount = 0
    iTotal = 0

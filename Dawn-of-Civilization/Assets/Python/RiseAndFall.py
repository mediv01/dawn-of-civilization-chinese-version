...
#####################################
    ...
    def checkTurn(self, iGameTurn):
        ...
        if not pCarthage.isHuman() or gc.getDefineINT("AIWAR_PY_HUMAN_CAN_USE_RISE_AND_FALL") == 1:  #修改为人类玩家也可以
            if iGameTurn == getTurnForYear(-820) - (data.iSeed % 10):
        ...
        if iGameTurn == getTurnForYear(-50):
            if pByzantium.isHuman() and pGreece.isAlive():
                ...
                if (gc.getDefineINT("PYTHON_LOG_ON_MAIN_RISE_AND_FALL") == 1):
                    utils.logwithid(iGreece, 'Start fall down')
        ...
        if iGameTurn == getTurnForYear(tBirth[iIndia]) - utils.getTurns(1):
            if pHarappa.isAlive() and not pHarappa.isHuman():
                ...
                if (gc.getDefineINT("PYTHON_LOG_ON_MAIN_RISE_AND_FALL") == 1):
                    utils.logwithid(iHarappa, 'Start fall down')
        ...
        if iGameTurn == getTurnForYear(-850):
            ...
            if (gc.getDefineINT("PYTHON_LOG_ON_MAIN_RISE_AND_FALL") == 1):
                utils.logwithid(iCarthage, 'Start Early Colonists')
        elif iGameTurn == getTurnForYear(-600):
            self.giveEarlyColonists(iRome)
            if (gc.getDefineINT("PYTHON_LOG_ON_MAIN_RISE_AND_FALL") == 1):
                utils.logwithid(iRome, 'Start Early Colonists')
        ...
        if utils.isYearIn(1200, 1918):  #欧洲殖民入口
        ...
        if iGameTurn == getTurnForYear(1700) - 2:
            if utils.getHumanID() >= iGermany and pAztecs.isAlive():
                ...
                if (gc.getDefineINT("PYTHON_LOG_ON_MAIN_RISE_AND_FALL") == 1):
                    utils.logwithid(iAztecs, 'Start fall down')
        ...
        for iLoopCiv in [iPlayer for iPlayer in range(iNumMajorPlayers) if tBirth[iPlayer] > utils.getScenarioStartYear()]:
            ...
            if iGameTurn >= getTurnForYear(tBirth[iLoopCiv]) - 2 and iGameTurn <= getTurnForYear(tBirth[iLoopCiv]) - 2:
                if (gc.getDefineINT("PYTHON_LOG_ON_MAIN_RISE_AND_FALL") == 1):
...
#参数控制
    ...
    def initBirth(self, iCurrentTurn, iBirthYear, iCiv):  # iBirthYear is really year now, so no conversion prior to function call - edead
        ...
        if iCiv in lConditionalCivs and utils.getHumanID() != iCiv and (not gc.getDefineINT("PY_RISE_ALL_CIV_CAN_BIRTH") == 1):
            if iCiv == iByzantium:
    ...
    def onFirstContact(self, iTeamX, iHasMetTeamY):
        ...
        if iGameTurn > getTurnForYear(600) and iGameTurn < getTurnForYear(1800):
            if iTeamX in lCivBioNewWorld and iHasMetTeamY in lCivBioOldWorld:
                ...
                if iGameTurn <= getTurnForYear(tBirth[iAztecs]):  #was getTurnForYear(tBirth[iAztecs])+10
                ...
                if not bAlreadyContacted:
                    ...
                    if lContactPlots and lArrivalPlots:
                        ...
                        if(tArrivalPlot):
                            ...
                            if (gc.getDefineINT("PYTHON_LOG_ON_MAIN_RISE_AND_FALL") == 1):
                                utils.logwithid(iOldWorldCiv, u'Start WAR with new world civ: '+gc.getPlayer(iNewWorldCiv).getCivilizationShortDescription(0))
...
#handi=(utils.getHumanID() == iMongolia and gc.getGame().getHandicapType()>=2)
        ihuman = gc.getDefineINT("AIWAR_PY_HUMAN_CAN_USE_AI_WAR") == 0 and utils.getHumanID() == iMongolia

        if iHasMetTeamY == iMongolia and not ihuman:  #修改让人类也享受加成
            if iTeamX in [iPersia, iRussia, iArabia, iByzantium]:  #移除了,
...
#						iDirection = DirectionTypes.DIRECTION_EAST
                    ...
                    lTargetList = utils.getBorderPlots(iTeamX, tTL, tBR, iDirection, 7)

                    if not lTargetList: return

                    teamMongolia.declareWar(iTeamX, True, WarPlanTypes.WARPLAN_TOTAL)
                    if (gc.getDefineINT("PYTHON_LOG_ON_MAIN_RISE_AND_FALL") == 1):
                        utils.logwithid(iMongolia, u'Start MONGOlIA WAR to '+str(gc.getPlayer(iTeamX).getCivilizationShortDescription(0)))
                    iHandicap = 0
                    if utils.getHumanID() == iTeamX:
                        iHandicap = round(gc.getGame().getHandicapType() / 2,0)
                    ...
                    for tPlot in lTargetList:
                        if (tPlot):
                            ...
                            '''
                            utils.log(str(iTeamX) + str(tPlot) + str(iMangudai))
                            utils.log(str(iTeamX)+str(tPlot)+str(iKeshik))
                            utils.makeUnitAI(iKeshik, iMongolia, tPlot, UnitAITypes.UNITAI_ATTACK_CITY, 2 + iHandicap)
                            utils.makeUnitAI(iMangudai, iMongolia, tPlot, UnitAITypes.UNITAI_ATTACK_CITY, 1 + 2 * iHandicap)
                            utils.makeUnitAI(iTrebuchet, iMongolia, tPlot, UnitAITypes.UNITAI_ATTACK_CITY, 1 + iHandicap)
                            '''
    ...
    def lateTradingCompany(self, iCiv):
        if utils.getHumanID() != iCiv and not utils.isAVassal(iCiv) and utils.getScenario() != i1700AD:
            if iCiv in [iFrance, iEngland, iNetherlands]:
                ...
                if (gc.getDefineINT("PYTHON_LOG_ON_MAIN_RISE_AND_FALL") == 1):
                    utils.logwithid(iCiv, 'Start late trade company')
    ...
    def earlyTradingCompany(self, iCiv):
        if utils.getHumanID() != iCiv and not utils.isAVassal(iCiv):
            if iCiv in [iSpain, iPortugal]:
                ...
                if (gc.getDefineINT("PYTHON_LOG_ON_MAIN_RISE_AND_FALL") == 1):
                    utils.logwithid(iCiv, 'Start early trade company')
        ...
        if utils.getHumanID() != iCiv:
            if iCiv == iAmerica:
                ...
                for iEnemy in lEnemyCivs:
                    ...
                    if (gc.getDefineINT("PYTHON_LOG_ON_MAIN_RISE_AND_FALL") == 1):
                        utils.logwithid(iCiv, 'Start railway war')
    ...
    def handleColonialAcquisition(self, iPlayer):
        ...
        for tPlot in targetList:
            ...
            if gc.getMap().plot(x, y).isCity():
                ...
                if not iTargetCiv in targetCivList:
                    ...
                    if (gc.getDefineINT("PYTHON_LOG_ON_MAIN_RISE_AND_FALL") == 1):
                        utils.logwithid(iPlayer,' Trade company war to '+str(gc.getPlayer(iTargetCiv).getCivilizationShortDescription(0)))
            else:
        ...
        for iTargetCiv in targetCivList:
            if iTargetCiv == utils.getHumanID():
                ...
                if (gc.getDefineINT("PYTHON_LOG_ON_MAIN_RISE_AND_FALL") == 1):
                    utils.logwithid(iPlayer, 'Start trade company war')
    ...
    def handleColonialConquest(self, iPlayer):
        ...
        if (gc.getDefineINT("PYTHON_RISEANDFALL_NO_TRADE_COMPANY_WAR") == 1) and utils.getHumanID() != iPlayer:  #mediv01
            targetList = []
            return
        for tPlot in targetList:
                        ...
                        if rndNum >= iBetrayalThreshold:
                            if unit.getDomainType() == DomainTypes.DOMAIN_LAND:
                                ...
                                if gc.getPlayer(data.iOldCivFlip).isHuman():
                                    ...
                                    if (gc.getDefineINT("PYTHON_ARMY_NOT_FLIP") == 1):  #参数控制是否翻转
                                        pass
                                    else:

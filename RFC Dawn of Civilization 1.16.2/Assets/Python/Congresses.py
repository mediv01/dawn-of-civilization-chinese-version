...
class Congress:
    ...
    def startClaimCityEvent(self):
        ...
        for tCity in self.dPossibleClaims[utils.getHumanID()]:
            ...
            if plot.isCity():
                ...
                popup.addPythonButton(poptext, gc.getCivilizationInfo(gc.getPlayer(plot.getPlotCity().getOwner()).getCivilizationType()).getButton())
            else:
    ...
    def applyBriberyEvent(self, iChoice, iBribedPlayer, iClaimant, iClaimValidity):
        if iChoice < len(self.lBriberyOptions):
            ...
            if iCommerceType == 0:
                gc.getPlayer(utils.getHumanID()).changeGold(-iCost)
            elif iCommerceType == 3:
                gc.getTeam(utils.getHumanID()).changeEspionagePointsAgainstTeam(iBribedPlayer, -iCost)
    ...
    def startCongress(self):
        ...
        Randomcity = utils.getRandomEntry(utils.getOwnedCoreCities(iHostPlayer, utils.getReborn(iHostPlayer)))
        if (Randomcity):
            self.sHostCityName = Randomcity.getName()
    ...
    def voteOnCityClaimAI(self, iVoter, iClaimant, tPlot, iClaimValue):
        ...
        if not bOwner:
            if iClaimValidity >= 0:
                if (gc.getDefineINT('PYTHON_CONGRESS_VOTE_FOR_EMPTY_TERROR_WITH_RANDOM') > 0):
                    iRand = gc.getGame().getSorenRandNum(100, 'Random vote outcome')
                    if iRand < 50:
                        print('Voted YES: random with empty territory')
                        if (gc.getDefineINT("PYTHON_LOG_ON_CONGRESS") == 1):
                            utils.log_congress(str(gc.getPlayer(iVoter).getCivilizationShortDescription(0)) + ' Voted YES: random with empty territory to ' + str(gc.getPlayer(iClaimant).getCivilizationShortDescription(0)))
                        self.vote(iVoter, iClaimant, 1)
                    else:
                        print('Voted NO: random with empty territory')
                        if (gc.getDefineINT("PYTHON_LOG_ON_CONGRESS") == 1):
                            utils.log_congress(str(gc.getPlayer(iVoter).getCivilizationShortDescription(0)) + ' Voted NO: random with empty territory to ' + str(gc.getPlayer(iClaimant).getCivilizationShortDescription(0)))
                        self.vote(iVoter, iClaimant, -1)
    ...
    def voteOnCityClaimAI_Prob(self, iVoter, iClaimant, tPlot, iClaimValue):
            ...
            if iClaimValidity >= 0:
                if (gc.getDefineINT('PYTHON_CONGRESS_VOTE_FOR_EMPTY_TERROR_WITH_RANDOM') > 0):
                    ...
                    if iRand < iClaimValidity:
                        print('Voted YES: random with empty territory')
                        if (gc.getDefineINT("PYTHON_LOG_ON_CONGRESS") == 1):
                            utils.log_congress_prob(str(gc.getPlayer(iVoter).getCivilizationShortDescription(0)) + ' Voted YES: random with empty territory to ' + str(gc.getPlayer(iClaimant).getCivilizationShortDescription(0)))
                        self.vote(iVoter, iClaimant, 1)
                    else:
                        print('Voted NO: random with empty territory')
                        if (gc.getDefineINT("PYTHON_LOG_ON_CONGRESS") == 1):
                            utils.log_congress_prob(str(gc.getPlayer(iVoter).getCivilizationShortDescription(0)) + ' Voted NO: random with empty territory to ' + str(gc.getPlayer(iClaimant).getCivilizationShortDescription(0)))
                        self.vote(iVoter, iClaimant, -1)
                    '''
                    ...
                    print('Voted YES: empty territory')
                    if (gc.getDefineINT("PYTHON_LOG_ON_CONGRESS") == 1):
                        utils.log_congress_prob(str(gc.getPlayer(iVoter).getCivilizationShortDescription(0)) + ' Voted YES: empty territory towards ' + str(gc.getPlayer(iClaimant).getCivilizationShortDescription(0)))
                    iProb += 1
                    #self.vote(iVoter, iClaimant, 1)
                return
        ...
        if bOwnClaim:
            if not bOwner or not bThreatenedOwner:
                print('Voted YES: own claim')
                if (gc.getDefineINT("PYTHON_LOG_ON_CONGRESS") == 1):
                    utils.log_congress_prob(str(gc.getPlayer(iVoter).getCivilizationShortDescription(0)) + ' Voted YES: own claim towards to ' + str(gc.getPlayer(iClaimant).getCivilizationShortDescription(0)))
                #self.vote(iVoter, iClaimant, 1)
                iProb += 1
                return
        ...
        if bOwner and bOwnCity:
            if not bThreatenedClaimant:
                print('Voted NO: claim on own city')
                if (gc.getDefineINT("PYTHON_LOG_ON_CONGRESS") == 1):
                    utils.log_congress_prob(str(gc.getPlayer(iVoter).getCivilizationShortDescription(0)) + ' Voted NO: claim on own city towards to ' + str(gc.getPlayer(iClaimant).getCivilizationShortDescription(0)))
                #self.vote(iVoter, iClaimant, -1)
                iProb -= 1
                return
        ...
        if bOwner and bMinor:
            if iClaimValidity > 0:
                print('Voted YES: valid claim on minors')
                if (gc.getDefineINT("PYTHON_LOG_ON_CONGRESS") == 1):
                    utils.log_congress_prob(str(gc.getPlayer(iVoter).getCivilizationShortDescription(0)) + ' Voted YES: valid claim on minors to ' + str(gc.getPlayer(iClaimant).getCivilizationShortDescription(0)))
                #self.vote(iVoter, iClaimant, 1)
                iProb += 1
            else:
                print('Voted NO: invalid claim on minors')
                if (gc.getDefineINT("PYTHON_LOG_ON_CONGRESS") == 1):
                    utils.log_congress_prob(str(gc.getPlayer(iVoter).getCivilizationShortDescription(0)) + ' Voted NO: invalid claim on minors to ' + str(gc.getPlayer(iClaimant).getCivilizationShortDescription(0)))
                #self.vote(iVoter, iClaimant, -1)
                iProb -= 1
            return
        ...
        if bOwner and not bOwnClaim:
            if tVoter.isAtWar(iClaimant) and gc.getTeam(iOwner).isAtWar(iClaimant) and not tVoter.isAtWar(iOwner):
                print('Voted NO: claimant is common enemy')
                if (gc.getDefineINT("PYTHON_LOG_ON_CONGRESS") == 1):
                    utils.log_congress_prob(str(gc.getPlayer(iVoter).getCivilizationShortDescription(0)) + ' Voted NO: claimant is common enemy to ' + str(gc.getPlayer(iClaimant).getCivilizationShortDescription(0)))
                #self.vote(iVoter, iClaimant, -1)
                iProb -= 1
        ...
        if iClaimValidity > 0 or (bOwner and bWarClaim):
            ...
            if iFavorClaimant + iClaimValidity < iFavorOwner:
                print('Voted NO: claimant favor and validity lower than owner favor')
                if (gc.getDefineINT("PYTHON_LOG_ON_CONGRESS") == 1):
                    utils.log_congress_prob(str(gc.getPlayer(iVoter).getCivilizationShortDescription(0)) + ' Voted NO: claimant favor and validity lower than owner favor to ' + str(gc.getPlayer(iClaimant).getCivilizationShortDescription(0)))
                #self.vote(iVoter, iClaimant, -1)
                iProb -= 1
            # valid claim and claimant is more liked
            elif iFavorClaimant > iFavorOwner:
                print('Voted YES: claimant favor higher than owner favor')
                if (gc.getDefineINT("PYTHON_LOG_ON_CONGRESS") == 1):
                    utils.log_congress_prob(str(gc.getPlayer(iVoter).getCivilizationShortDescription(0)) + ' Voted YES: claimant favor higher than owner favor to ' + str(gc.getPlayer(iClaimant).getCivilizationShortDescription(0)))
                #self.vote(iVoter, iClaimant, 1)
                iProb += 1
            # less liked, but justified by claim
            elif iFavorClaimant + iClaimValidity >= iFavorOwner:
                ...
                if (iClaimant == utils.getHumanID() or (bOwner and iOwner == utils.getHumanID())) and iClaimValidity < 50 and iFavorOwner - iFavorClaimant > 0:
                    ...
                    print('NO VOTE: open for bribes')
                    if (gc.getDefineINT("PYTHON_LOG_ON_CONGRESS") == 1):
                        utils.log_congress_prob(str(gc.getPlayer(iVoter).getCivilizationShortDescription(0)) + ' NO VOTE: open for bribes to ' + str(gc.getPlayer(iClaimant).getCivilizationShortDescription(0)))
                    pass
                else:
                    ...
                    if iRand < iClaimValidity:
                        print('Voted YES: random')
                        if (gc.getDefineINT("PYTHON_LOG_ON_CONGRESS") == 1):
                            utils.log_congress_prob(str(gc.getPlayer(iVoter).getCivilizationShortDescription(0)) + ' Voted YES: random to ' + str(gc.getPlayer(iClaimant).getCivilizationShortDescription(0)))
                        self.vote(iVoter, iClaimant, 1)
                    else:
                        print('Voted NO: random')
                        if (gc.getDefineINT("PYTHON_LOG_ON_CONGRESS") == 1):
                            utils.log_congress_prob(str(gc.getPlayer(iVoter).getCivilizationShortDescription(0)) + ' Voted NO: random to ' + str(gc.getPlayer(iClaimant).getCivilizationShortDescription(0)))
                        self.vote(iVoter, iClaimant, -1)
                    '''
            ...
            if iFavorClaimant + iClaimValidity > iFavorOwner:
                print('Voted YES: likes claimant enough despite bad claim')
                if (gc.getDefineINT("PYTHON_LOG_ON_CONGRESS") == 1):
                    utils.log_congress_prob(str(gc.getPlayer(iVoter).getCivilizationShortDescription(0)) + ' Voted YES: likes claimant enough despite bad claim to ' + str(gc.getPlayer(iClaimant).getCivilizationShortDescription(0)))
                #self.vote(iVoter, iClaimant, 1)
                iProb += 1
            else:
                print('Voted NO: bad claim')
                if (gc.getDefineINT("PYTHON_LOG_ON_CONGRESS") == 1):
                    utils.log_congress_prob(str(gc.getPlayer(iVoter).getCivilizationShortDescription(0)) + ' Voted NO: bad claim to ' + str(gc.getPlayer(iClaimant).getCivilizationShortDescription(0)))
                #self.vote(iVoter, iClaimant, -1)
                iProb -= 1
